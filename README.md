...
1. What are the main differences between a stateful and a functional component?
   Stateful components is basically a component or a section in react that will store our states. Its can also use props.
   In regards to stateless components it does not hold or have any state. Even though stateless components does not hold any state; it too can also use props.
   Also with functional components it is able to take props and return to use JSx but is does not have any lifecycle methods.

2. When does a componentWillMount function be called? What about a componentWillUpdate?
   A componentWillMount is called before the render method is executed whiles the componentWillUpdate is called right before it takes a new state or props.

3. Define stateful logic.
   stateful logic uses state and includes hook and custom hook with state, use behavior to change state.

4. What are the three step of creating a successful test? What is done in each phase?
   we have the first step which is to ARRANGE then ACT and lastly we have ASSERT.
   Arrange will set up our inputs
   act will cover our main test
   assert will insure our expected or end results
